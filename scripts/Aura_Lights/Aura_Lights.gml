	    ...
	    if !surface_exists(aura_light_surface)
	        ...
	        if aura_light_renew && aura_in_view(_v)
	            ...
	            with(obj_Aura_ShadowCaster_Parent)
	                {
	                if aura_shadow_enabled and aura_in_light(a_inst)
	                    {
                        controller.shadow_casters_count++
	                    // Loop through the shadow caster vertices and add them to the vertex buffer
	                    for(i = 0; i < aura_shadow_points; i++;)
	                        {
	                        a_tx[i] = x + aura_shadow_x[i] - a_ox + a_rad;  // point position relative
	                        a_ty[i] = y + aura_shadow_y[i] - a_oy + a_rad;  // to light center ON SURFACE (which has (a_rad, a_rad) coords on surface)
	                        a_dir = point_direction(a_tx[i], a_ty[i], a_rad, a_rad) + 180;
	                        a_px[i] = a_tx[i] + lengthdir_x(aura_shadow_length, a_dir);   // casted shadow's extension
